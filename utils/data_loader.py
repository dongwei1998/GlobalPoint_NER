...
def load_data(file_path,ent2id_path):
    with open(ent2id_path, 'r', encoding='utf-8') as r1:
        ent2id = json.load(r1)
        id2ent = {}
        for k, v in ent2id.items(): id2ent[v] = k
    D = []
    for d in json.load(open(file_path,encoding='utf-8')):
        D.append([d['text']])
        for e in d['entities']:
            start, end, label = e['start_idx'], e['end_idx'], e['type']
            if start <= end:
                D[-1].append((start, end, ent2id[label]))
    return D,id2ent,ent2id



def load_data_v1(file_path,ent2id_path):
    with open(ent2id_path,'r',encoding='utf-8') as r1:
        ent2id = json.load(r1)
        id2ent = {}
        for k, v in ent2id.items(): id2ent[v] = k
...
class EntDataset(Dataset):
    def __init__(self, data,ent2id, tokenizer, istrain=True):
        self.data = data
    ...
    def collate(self, examples):
        ...
        for item in examples:
            ...
            labels = np.zeros((len(self.ent2id), max_len, max_len))
...
if __name__ == '__main__':
    load_data('../datas/dianfei/train_data.json', './datasets/dianfei/ent2id_data.json')
